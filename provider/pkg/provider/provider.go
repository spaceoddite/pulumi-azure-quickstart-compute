...
func construct(ctx *pulumi.Context, typ, name string, inputs provider.ConstructInputs,
	...
	switch typ {
	case "azure-quickstart-compute:index:virtualmachine":
		return constructvirtualmachine(ctx, name, inputs, options)
	default:
...
func constructvirtualmachine(ctx *pulumi.Context, name string, inputs provider.ConstructInputs,
	options pulumi.ResourceOption) (*provider.ConstructResult, error) {
	...
	virtualmachine, err := Newvirtualmachine(ctx, name, args, options)
	if err != nil {
	...
	return provider.NewConstructResult(virtualmachine)
}
